...
#         self.save_folder = os.path.join(PDT.get_prop_db_folder(), self.name)
...
#                 else:
#                     PDT.PDT_Error(ValueError, 'Unknown KWARG input "{}"'.format(key))
#
...
#         self.xrotor_op_dict = PDT.read_xrotor_op_file(fpath=self.xrop_file)
#
#         # and finally read in the point cloud files
#         self.blade_xyz_profiles = {}
#         fnames = PDT.search_files(folder=self.bld_prof_folder)
#         for fname in fnames:
#             prof_num = int(fname.replace('profile_', '').replace('.txt', ''))
#             xyz_prof = PDT.read_profile_xyz(fpath=os.path.join(self.bld_prof_folder, fname))
#             self.blade_xyz_profiles[prof_num] = xyz_prof
#
#         return
#
#     def set_stations(self, plot_also: bool = True, verbose: bool = False):
#         self.stations, txt = PDT.create_radial_stations(prop=self, plot_also=plot_also,
#                                                         verbose=verbose)
#         return txt
#
...
#             PDT.PDT_Error(NotImplementedError, '> 1 profile interpolation not yet implemented')
#
#         if tot_skew != 0.0:
#             PDT.PDT_Warning('Blade "skew" is not implemented in XROTOR, and therefore not reflected in XROTOR results.\n'
#                             '  > Skew effects are considered negligible for PDT purposes for small enough skew angles.')
#
#         # clear out the existing xyz profiles
#         PDT.delete_files_from_folder(self.bld_prof_folder)
#
...
#             r = roR * self.radius
#
#             prof_xyz = PDT.generate_3D_profile_points(
#                 nondim_xy_coords=nondim_coords, radius=r, axis_shift=0.25, chord_len=chord, beta_deg=beta, skew_deg=skew)
...
#                     xc, yc, zc = PDT.generate_3D_profile_points(
#                         nondim_xy_coords=foil.get_coords(), radius=r, axis_shift=0.25, chord_len=ch, beta_deg=beta,
#                         skew_deg=sk)
...
#             PDT.PDT_Info('Saved PNG to "{}"'.format(savepath))
#
...
#         if verbose:
#             PDT.PDT_Info('Saved STL file and reloaded into propeller object: "{}"'.format(self.stl_fpath))
#
#         if plot_after:
#             self.plot_stl_mesh()
#
