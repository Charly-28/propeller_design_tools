...
#                  Xisection: float = None, plot: bool = False, **xrotor_kwargs):
...
#                     raise KeyError('Required argument "{}" not given'.format(attr))
...
#             self.init_from_airfoil(foil=foil, re_estimate=re_estimate, plot_also=plot)
#
...
#         idx_lims = self.calc_xrotor_fit_params(pol=pol)
#         if plot_also:
...
#         return CD
#
#     def calc_xrotor_fit_params(self, pol: dict):
...
#         popt, pcov = curve_fit(f=PDT.xrotor_drag_model, xdata=xs, ydata=ys, bounds=([0.0, -1.0, 0.0], [0.5, 2.5, .5]))
#         self.CDmin, self.CLCDmin, self.dCDdCL2 = popt
#
#         # pitching moment
#         self.Cmconst = np.average(pol['CM'][min_idx:pre_stall_idx])
#         self.REref = pol['re']
#         self.REexp = PDT.get_xrotor_re_scaling_exp(re=pol['re'])
#         self.Mcrit = 1
#
...
#                      format(self.station_idx + 1, self.Xisection, self.foil.name, pol['re'], pol['mach'], pol['ncrit']))
#
